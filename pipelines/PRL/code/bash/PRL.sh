...
if [ "$step" == "preparation" ]; then
  ...
  if [ "$mode" == "batch" ]; then
    ...
    do 
        ...
        do
          ...
          if [ "$c" == "cluster" ]; then
            ...
            --participant $p --session $s --t1 $t1_r --flair $flair_r --phase $phase_r --n4 $n4 --skullstripping $skullstripping --stype $stype \
    ...
    if [ "$c" == "cluster" ]; then
            ...
            Rscript $tool_path/pipelines/PRL/code/R/PRL.R --mainpath $main_path \
            --participant $p --session $ses --t1 $t1_r --flair $flair_r --phase $phase_r --n4 $n4 --skullstripping $skullstripping --stype $stype \
            --registration $registration --whitestripe $whitestripe --mimosa $mimosa --threshold $threshold \
