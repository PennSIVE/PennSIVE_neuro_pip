    ...
    do 
        ...
        do
          ...
          if [ "$c" == "cluster" ]; then
            ...
            docker run --rm -it -v $main_path:/home/main -v $tool_path:/home/tool $docker_path Rscript $tool_path/pipelines/alpaca/alpaca_mod.R --mainpath $main_path \
            --participant $p --session $s --t1 $t1_r --flair $flair_r --epi_mag $epimag_r --epi_phase $epipha_r --n4 $n4 --skullstripping $skullstripping \
            --registration $registration --whitestripe $whitestripe --mimosa $mimosa --threshold $threshold \
            --hdbetpath $hdbet_path --lesioncenter $tool_path/lesioncenter --mpath $tool_path/pipelines/mimosa/model/mimosa_model.RData --helpfunc $tool_path/help_functions > /home/main/log/output/alpaca_output_${p}_${s}.log 2> /home/main/log/error/alpaca_error_${p}_${s}.log
          fi
        done
    done
