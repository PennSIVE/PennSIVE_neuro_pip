...
show_help() {
  ...
  echo "  --dockerpath   Specify the path to the docker image if a docker container is used. Note - running on MacOS, be sure the memory limit of the VM is sufficiently large. A default path is provided: russellshinohara/pennsive_arm64_cputorch:v1.1"
  echo "  --hdbetpath   Specify the path to the HD-BET binary. For the Docker container, this is hard-coded in this version. A default path is provided: ~/.local/bin/hd-bet"
